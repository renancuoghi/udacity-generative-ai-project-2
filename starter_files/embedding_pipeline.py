...
class ChromaEmbeddingPipelineTextOnly:
    ...
    def __init__(self, 
        ...
        try:
            ...
            # Using basic collection creation without built-in function to allow manual embedding control
    ...
    def chunk_text(self, text: str, metadata: Dict[str, Any]) -> List[Tuple[str, Dict[str, Any]]]:
        ...
        # Validate chunk parameters to prevent infinite loops
        if self.chunk_overlap >= self.chunk_size:
            logger.warning(f"chunk_overlap ({self.chunk_overlap}) >= chunk_size ({self.chunk_size}). Setting overlap to chunk_size - 1")
            effective_overlap = self.chunk_size - 1
        else:
        ...
        while start < text_len and chunk_index < max_chunks:
            ...
            if end < text_len:
                # Look for last period, newline, or question mark within the last 100 chars of the chunk
                # to avoid breaking sentences mid-way
                lookback_size = min(100, end - start)  # Don't look back more than chunk size
                if lookback_size > 0:
                    lookback = text[end-lookback_size:end]
                    last_period = lookback.rfind('.')
                    last_newline = lookback.rfind('\n')
                    
                    break_point = max(last_period, last_newline)
                    if break_point != -1:
                        # Adjust end to just after the punctuation
                        end = (end - lookback_size) + break_point + 1
            ...
            if chunk_text: # Only add non-empty chunks
                ...
                # Total chunks is hard to know upfront with dynamic boundaries, 
                # but we can update it later or leave it as partial info
            ...
            # Ensure strict forward progress - must move at least 1 character forward
            if next_start <= start:
                next_start = start + 1
        ...
        if chunk_index >= max_chunks:
            logger.error(f"Hit maximum chunk limit ({max_chunks}). Text may be too large or chunking parameters invalid.")
    ...
    def generate_document_id(self, file_path: Path, metadata: Dict[str, Any]) -> str:
        ...
        # Clean source name to be safe for ID
        safe_source = source.replace(' ', '_').replace('.', '_')
        
        return f"{mission}_{safe_source}_chunk_{chunk_idx:04d}"
    ...
    def add_documents_to_collection(self, documents: List[Tuple[str, Dict[str, Any]]], 
        ...
        for i in range(0, len(documents), batch_size):
            ...
            for text, meta in batch_docs:
                ...
                # If we are here, we need to add or update
                embedding = self.get_embedding(text)
                if not embedding:
                    continue
            ...
            if ids:
                try:
                    ...
                    # Rollback stats count if failed? Or just log.
                    # Simple count correction roughly:
                    stats['added'] -= len([x for x in ids if not self.check_document_exists(x)]) # approximate
    ...
    def process_all_text_data(self, base_path: str, update_mode: str = 'skip') -> Dict[str, int]:
        ...
        for file_path in files:
            try:
                ...
                m_stats = stats['missions'][mission]
                m_stats['files'] += 1
                m_stats['chunks'] += len(chunks)
                m_stats['added'] += file_stats['added']
                m_stats['updated'] += file_stats['updated']
                m_stats['skipped'] += file_stats['skipped']
